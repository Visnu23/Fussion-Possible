...
# ==========================================================
# PATHS - EXACTLY FROM YOUR WORKING MODEL
# ==========================================================

BASE_PATH = r"D:\python 3.10.7\home work\documents\projects\Major Project\Agriculture app\make a try"
MODEL_PATH = os.path.join(BASE_PATH, "resnet50_fusion.pt")
CLASSES_PATH = os.path.join(BASE_PATH, "classes.txt")
ADVICE_PATH = os.path.join(BASE_PATH, "advice.json")
...
def load_fusion_model():
    """Load the ResNet-50 fusion model"""
    try:
        # Check if model exists
        if not os.path.exists(MODEL_PATH):
            st.error(f"❌ Model not found at: {MODEL_PATH}")
            st.info("Please train the model first using the provided notebook.")
            return None, None
        ...
        st.sidebar.success(f"✅ Fusion Model loaded successfully")
...
# ==========================================================
# INFERENCE TRANSFORMS - EXACTLY FROM YOUR WORKING MODEL
# ==========================================================
...
def predict_image(pil_img: Image.Image, model, classes):
    """Predict image class and automatically determine type"""
    ...
    # Decide type directly from predicted class
    ...
    # Compute green ratio
    green_ratio = compute_green_ratio(pil_img)

    # PROFESSIONAL GATING LOGIC
    # Force leaf if strong green signal
    if label1 in soil_classes and green_ratio > 0.25:
        leaf_classes = [c for c in classes if c not in soil_classes]
        leaf_probs = [(idx, probs[idx].item()) for idx in range(len(classes)) if classes[idx] in leaf_classes]
        leaf_probs.sort(key=lambda x: x[1], reverse=True)
        top1_idx, conf1 = leaf_probs[0]
        label1 = classes[top1_idx]

        # Likely soil misclassified as leaf
        soil_indices = [i for i, c in enumerate(classes) if c in soil_classes]
        soil_probs = probs[soil_indices]

        best_soil_idx = soil_indices[np.argmax(soil_probs)]
        label1 = classes[best_soil_idx]
        conf1 = float(probs[best_soil_idx])

        image_type = "Soil"
        sev_score = 0.0
        stage = "N/A"
        
    elif label1 in soil_classes:
        image_type = "Soil"
        sev_score = 0.0
        stage = "N/A"
...
def generate_gradcam(model, img, class_idx):
    ...
    try:
        ...
        image_tensor = infer_tfm(img).unsqueeze(0).to(device)
...
def main():
    ...
    with st.sidebar:
        ...
        if model is not None:
            ...
            st.markdown(f"""
            - **Fusion Model:** ❌ Offline
            - **Status:** Please train model first
            """)
    ...
    # Model check
    if model is None:
        st.error("⚠️ Model not loaded. Please train the model first using the provided notebook.")
    ...
    with tab3:
        ...
        with col2:
            ...
            st.metric("Architecture", "ResNet-50")
